...
if (isset($_GET['exerciseName'])) {
    ...
    // Prepare the SQL statement to remove the exercise preference
    $stmt = $conn->prepare("DELETE FROM likes WHERE user_id = ? AND exercise = ?");
    ...
    if ($stmt->execute()) {
        ...
        $response = array("success" => true, "message" => "Exercise preference removed successfully");
        echo json_encode($response);
    } else {
